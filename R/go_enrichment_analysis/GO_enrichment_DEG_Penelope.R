...
penelope = read.csv('/Users/alenakizenko/Documents/PhD/project/hydra_vulgaris_chrom/genome_annotation_Penelope/Penelope_gene_annotation.tsv',
...
#---------all genes near Penelope repeats--------------------

genesOfInterest <- as.character(unique(penelope$Gene))
geneList <- factor(as.integer(geneUniverse %in% genesOfInterest))
names(geneList) <- geneUniverse
myGOdata <- new("topGOdata", description="My project", ontology="BP", allGenes=geneList,  annot = annFUN.gene2GO, gene2GO = geneID2GO)
test.stat <- new("parentChild", testStatistic = GOFisherTest, name = "Fisher test")
...
go_penelope_hvul = ggplot(allRes, aes(reorder(Term, Significant), -log10(classicFisher),
                                  fill = Significant)) +
...
ggsave(plot = go_penelope_hvul, filename = '/Users/alenakizenko/Documents/PhD/project/pictures/go_penelope_cc_hvul.tiff',
