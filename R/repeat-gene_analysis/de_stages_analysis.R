...
expan = read.csv('/Users/alenakizenko/Documents/PhD/project/hydra_vulgaris_chrom/genome_annotation_CR1/LINE_CR1_expansion_gene_annotation.tsv',
                 stringsAsFactors = F, header = F, sep = '\t')
View(head(expan))
rst = read.csv('/Users/alenakizenko/Documents/PhD/project/hydra_vulgaris_chrom/genome_annotation_CR1/LINE_CR1_rest_gene_annotation.tsv',
               stringsAsFactors = F, header = F, sep = '\t')
...
de_norm_rep_3h = subset(de_norm_rep, DE_type == 'Up 3h' | DE_type == 'Down 3h')
str(de_norm_rep_3h)

deg_3 = ggplot(de_norm_rep_3h, aes(DE_type, Repeats_num_near_deg*100,
                              label = Number_of_repeats,
                              fill = Repeat_type)) +
  geom_bar(stat = 'identity', position = 'dodge', color = 'black'
           #fill = brewer.pal(8, 'Pastel1')[7]
           ) +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0, size = 18, face = 'bold'),
        axis.text.x = element_text(size=18,color = "black", angle = 40, hjust = 1),
...
df = data.frame(Type = c('Up new', 'Up old', 'Down new', 'Down old'),
           Number = c(9.097967e-06, 1.074691e-05, 6.531874e-06, 1.653371e-05))

ggplot(df, aes(Type, Number)) +
